...
A child-safe phone line, built two ways on SignalWire. A parent keeps a whitelist of
who may call the child and who the child may call. The application never answers a
call to make that decision: an allowed caller is bridged the moment the child picks
up, a blocked caller never sees a connected state, and billing starts at the bridge.

Both implementations sit behind one test lab you drive from a real cell phone.

## Where the SignalWire code is

Each app keeps its SignalWire integration in a `signalwire/` folder. If you read
nothing else, read these ten files. Everything outside them is plumbing.
...
| `handler.js` | The heart: receive the offer, authorize while the leg stays unanswered, then `call.connect` with the child as the peer, or decline. The app never calls `answer()` |
| `connect-guard.js` | Containment for SDK realities on this package: re-offer storms during slow lookups, duplicate collapse, one connect or one decline per call |
...
```
apps/relay/          RELAY Realtime implementation
  src/signalwire/      the SDK integration (read this)
  src/                 supporting logic
  live/                live-call harness: provisioning, smoke rows, guided listener
  test/                unit tests (73)
